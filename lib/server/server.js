...
class Server {
  ...
  async listener(req, res) {
    ...
    if (this.balancer) {
      const host = util.parseHost(req.headers.host);
      const port = util.sample(this.config.ports);
  ...
  async close() {
    ...
    if (channels.size === 0) {
      await util.delay(SHORT_TIMEOUT);
      return;
    }
    await util.delay(this.config.timeouts.stop);
    this.closeChannels();
  }
}
